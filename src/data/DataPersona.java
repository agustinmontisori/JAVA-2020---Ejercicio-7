...
public class DataPersona {
	...
	public LinkedList<Persona> getByApellido(Persona perApe) {
		DataRol dr=new DataRol();
		LinkedList<Persona> perso= new LinkedList<>();
		PreparedStatement stmt=null;
		ResultSet rs=null;
		try {
			stmt=DbConnector.getInstancia().getConn().prepareStatement(
			...
			stmt.setString(1, perApe.getApellido());
	...
	public void edit(Persona p) {
		...
		try {
			stmt=DbConnector.getInstancia().getConn().prepareStatement(
							"update persona set tipo_doc=?, nro_doc=?, nombre=?,apellido=?, email=?, password=?, tel=?, habilitado=? where id = ?",
							PreparedStatement.RETURN_GENERATED_KEYS
							);
